...
def get_eml_content(filename):
    ...
    if msg.is_multipart():
        for payload in msg.get_payload():
            part_msg = payload.get_payload(decode=True)
            rm_tag_msg = bs4.BeautifulSoup(part_msg, "lxml").text
            content.append(rm_tag_msg)
    else:
        whole_msg = msg.get_payload(decode=True)
        rm_tag_msg = bs4.BeautifulSoup(whole_msg, "lxml").text
        content.append(rm_tag_msg)
